...
export default function CustomerSettingPage() {
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-[#f0f4f0]">
            <p className="text-black-400 text-sm">Loading...</p>
        </div>
    )
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#f0f4f0]">
            ...
            {/* ─── HEADER ─────────────────────────────────────────── */}
            <header className="w-full bg-white px-6 py-3 flex items-center gap-4 border-b border-black-100 sticky top-0 z-30">
                <Link href="/" className="shrink-0">
                    <Image src="/logo-plateup.png" alt="PlateUp!" width={110} height={34} className="object-contain" />
                </Link>
                <div className="flex-1 max-w-xl mx-auto">
                    <div className="flex items-center bg-black-50 border border-black-200 rounded-full px-4 py-2 gap-2">
                        ...
                        <input type="text" placeholder="Find your food..."
                            className="bg-transparent text-sm outline-none flex-1 text-black-700 placeholder-black-400" />
                    </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className="text-sm font-medium text-black-800 hidden md:block">{firstName}</span>
                </div>
            </header>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                {/* ─── SIDEBAR ──────────────────────────────────────── */}
                <aside className="w-52 bg-white border-r border-black-100 flex flex-col py-5 px-3 shrink-0">
                    <div className="flex items-center gap-2 px-2 mb-5">
                        <div className="w-9 h-9 rounded-full bg-[#d4e8c2] flex items-center justify-center text-[#3a7d44] font-bold text-sm">
                            {initials}
                        </div>
                        <div className="min-w-0">
                            <p className="text-sm font-medium text-black-900 truncate">Hello, {firstName}!</p>
                            <p className="text-xs text-black-400">Ready to save the planet?</p>
                        </div>
                    </div>
                    ...
                    ${isActive
                                            ...
                                            : 'text-black-600 hover:bg-black-50 hover:text-black-900'
                                        }`}
                                >
                                    <Icon size={18} strokeWidth={isActive ? 2.5 : 2}
                                        className={isActive ? 'text-[#2d5a1a]' : 'text-black-500'} />
                                    {item.label}
                                </Link>
                            )
                        })}
                    </nav>
                    <div className="mt-auto pt-4 border-t border-black-100">
                ...
                <main className="flex-1 overflow-y-auto p-6">
                    <div className="max-w-3xl mx-auto">
                        ...
                        <div className="mb-5">
                            <h1 className="text-xl font-bold text-black-900">Profile & Preference</h1>
                            <p className="text-sm text-black-500">Manage your profile and preferences details.</p>
                        </div>

                        {/* Account Profile Card */}
                        <div className="bg-white rounded-2xl border border-black-100 shadow-sm p-6 mb-4">
                            <h2 className="font-semibold text-black-800 mb-4">Account Profile</h2>
                            {[
                                ...
                                <div key={field.key} className="mb-4">
                                    <label className="block text-sm text-black-500 mb-1">{field.label}</label>
                                    <input
                                        type={field.type}
                                        value={profile[field.key as keyof typeof profile]}
                                        disabled={field.disabled}
                                        onChange={e => setProfile(prev => ({ ...prev, [field.key]: e.target.value }))}
                                        className="w-full border-b border-black-200 py-1.5 text-sm outline-none
                               focus:border-[#3a7d44] disabled:text-black-400 bg-transparent"
                                    />
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-black-100 shadow-sm p-6 mb-6">
                            <h2 className="font-semibold text-black-800 mb-1">Food Preferences</h2>
                            <p className="text-sm text-black-400 mb-3">I'm Craving...</p>
                      ...
                      ${selectedPrefs.includes(pref)
                        ...
                        <div className="flex items-end justify-between">
                            <div>
                                {lastEdited && (
                                    <>
                                        <p className="text-xs text-black-400">Last Edited</p>
                                        <p className="text-xs text-black-500 font-medium">{lastEdited}</p>
                                    </>
                                )}
                            </div>
                            <div className="flex gap-3">
                                <button onClick={() => window.location.reload()}
                                    className="px-5 py-2.5 text-sm font-semibold text-black-600 border border-black-200 rounded-xl hover:bg-black-50">
                                    Discard
                                </button>
